...
Why does it only work when placed in "examples" folder?
...
class Commander(cmd.Cmd):
	...
	if autoplay == "y":
		print "Autoplay on"
		print "Uri: " + uri
		print "Saved Track: " + savedtrack
		print "Trackindex: " + str(trackindex)
		print "Playlistnr: " + str(playlistnr)
	...
	for line in tracks:
		items+=1
	print "Playlist contains " + str(items) + " tracks."
	
	
	self.do_play(savedtrack)
	
	
	"""
	global cleantrack
	global trackindex		# Track number in playlist
	trackindex=0
	cleantracks = tracks[trackindex].split("'")
	cleantrack = cleantracks[1]
	print "Cleantrack:" + cleantrack
	
	self.do_play(cleantrack)
	"""
	"""
	...
	"loadpl <pnr>"
	playlistnr = usersplaylists[0][int(list)]
	...
	print "Save current playlist. Uri: " + uri
	...
	config.set('playlist', 'playlistnr', playlistnr)
    ...
    def do_nextpl(self,line):
	global playlistnr
	print "Current list: " + str(playlistnr)
	playlistnr+=1
	self.do_loadpl(str(playlistnr))
	...
	if trackindex < items:
		trackindex+=1
		uri = geturifromindex(trackindex)
		print uri
		self.do_play(uri)
	else:
	...
	if trackindex < items:
		trackindex+=1
		print "Go to track " + str(trackindex) + " of " + str(items)
		cleantracks = tracks[trackindex].split("'")
		cleantrack = cleantracks[1]
		print "Next track:" + cleantrack
		self.do_play(cleantrack)
	else:
		print "End of playlist"
    ...
    def do_play(self, curtrack):
	global trackindex
	global savetrack
	savetrack = curtrack
        print "In do_play: Track to play: " + curtrack
	
	track = self.session.get_track(curtrack)
	track.load()
	...
	artists = artists.split("'")
	realartist = self.session.get_artist(artists[1])
	print str(trackindex) + ":" +  str(realartist.name) + " - " + realtrack
...
def geturifromindex(trackindex):
	print "geturifromindex"
	cleantracks = tracks[trackindex].split("'")
	cleantrack = cleantracks[1]
	return cleantrack	
...
if __name__ == '__main__':
	...
	try: 
		...
		global uri
		global savetrack
		#curplaylist = session.get_artist(artists[1])
		print "In exit, saving info"
		print "Trackindex: " + str(trackindex)
		print "Playlist: " + uri
		print "Track: " + savetrack
		...
		try: 
			...
			pass
		config.set('CurrentTrack', 'track',  savetrack)
		config.set('CurrentTrack', 'trackindex',  str(trackindex))		
	
		with open('settings', 'w') as f:
			config.write(f)
		
		# The playlist uri is saved when user changes playlist
